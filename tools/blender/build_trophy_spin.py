# 冠军奖杯 360° 旋转帧序列渲染脚本（无头运行）
# 基于 tools/blender/build_trophy.py 改写：奖杯部件挂到 Empty 上绕 Z 轴旋转
# 输出: assets/trophy/spin/trophy_spin_00.png ~ trophy_spin_15.png（256×256 RGBA）
import bpy
import math
import os
from mathutils import Vector

OUT_DIR = "E:/workspace/godot/poker/assets/trophy/spin"
OUT_PATTERN = os.path.join(OUT_DIR, "trophy_spin_{:02d}.png")
FRAMES = 16
ANGLE_STEP = 2.0 * math.pi / FRAMES  # 22.5°
...
# 旋转根节点：所有奖杯部件 parent 到 Empty，转 Empty 实现整体旋转
root = bpy.data.objects.new("TrophyRoot", None)
scene.collection.objects.link(root)
for obj in (base, cup, handle_r, handle_l, plaque):
    obj.parent = root
...
for frame in range(FRAMES):
    root.rotation_euler = (0.0, 0.0, frame * ANGLE_STEP)
    out_path = OUT_PATTERN.format(frame)
    scene.render.filepath = out_path
    bpy.ops.render.render(write_still=True)
    print(f"render done [{frame:02d}/{FRAMES - 1}] angle={math.degrees(frame * ANGLE_STEP):.1f} -> {out_path}")

# ---------- 8. 构图校验：每帧 alpha 包围盒占比 ----------
def check_bbox(path):
    ...
    cx = (min_x + max_x) / 2 / w
    cy = (min_y + max_y) / 2 / h
    return (f"宽占比={bw:.3f} 高占比={bh:.3f} 中心=({cx:.3f},{cy:.3f}) "
            f"(0.5,0.5 为居中)")

for frame in range(FRAMES):
    print(f"BBOX[{frame:02d}]: {check_bbox(OUT_PATTERN.format(frame))}")
