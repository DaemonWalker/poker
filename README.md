...
- 8 个固定身份 AI：各有一套独立打法参数（紧度/激进度/跟注倾向/诈唬/风险/位置意识）与情绪规则，输大锅会"上头"（报复性变松或收紧），座位旁浮 emoji 气泡提示
- 完整外围流程：主菜单 → 新建/继续锦标赛 → 牌桌 → 结算名次表 → 战绩统计 → 再来一局
- 观战模式：全 AI 同桌对战、玩家手牌全程明牌、每个在局玩家实时胜率（翻牌前蒙特卡洛渐进收敛，翻牌圈起精确枚举）；不写存档、不计战绩
...
# 观战模式无头冒烟（全 AI 打完整场后自动退出，不写存档/战绩）
...
core/ai/    AI 决策器、牌力启发式评分、身份参数预设、情绪状态（tilt）
