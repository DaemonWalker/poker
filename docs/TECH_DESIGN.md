...
- **AIProfiles**：外观与打法解耦。`IDENTITIES` 为 8 个固定显示身份（石头/疯子/老枪/秤砣/狐狸/鲨鱼/木头/浪人，各配 `avatar_*` id），只决定名字与头像；`PARAM_POOL` 为 24 组打法参数表（键 `"P01"~"P24"`），开局每个 AI 带重复随机抽一组，`PlayerState.ai_profile` 存参数组键。每组 11 参数——**静态风格 6 项**（`tightness` 紧度 / `aggression` 激进度 / `calling_tendency` 跟注倾向 / `bluff_frequency` 诈唬频率 / `risk_tolerance` 风险承受 / `position_awareness` 位置意识）+ **情绪规则 4 项**（`tilt_sensitivity` / `tilt_recovery` / `tilt_looseness_dir` / `tilt_bluff_dir`，数值见文件内注释）+ **对手建模 1 项**（`adaptability` 对手调制幅度 0~1，0 = 不学习，池内 0~0.95 不等）。`PROFILES`（8 张具名参数表）保留作风格基准：缺省回落（`DEFAULT_PROFILE = "鲨鱼"`）、测试注入与旧存档兼容；`get_profile(id)` 先查 `PROFILES` 再查 `PARAM_POOL`。
...
场景结构（table.tscn）：`TableFelt`（桌布贴图）+ `CommunitySlots/Slot0..4` + `DeckSlot`（发牌动画起点）+ `UILayer`（CanvasLayer：TopBar、MenuButton、StreetLabel、PotLabel、MessageLabel、BlindBanner、ActionPanel）。座位不再用手摆槽位，由 `_seat_position(i, N)` 沿椭圆（`SEAT_CENTER(640, 324)`、`SEAT_RADIUS(500, 205)`）等角均匀分布，0 号人类座位固定正下方；半径按 SeatUI 面板尺寸（170×126）调过，9 人满桌时两侧座位纵向间距 ≥ 面板高度、底座位不压 ActionPanel（校验工具 `tools/check_layout.gd`）。桌面中央下注区为常量 `POT_POS(640, 300)`。
